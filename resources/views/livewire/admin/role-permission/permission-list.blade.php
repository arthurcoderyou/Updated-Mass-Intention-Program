                        ...
                        @foreach ($permissions as $permission)
                            ...
                            <tr>
                                ...
                                <td class="h-px w-auto whitespace-nowrap">
                                    <div class="px-6 py-2">
                                        <span class="text-sm text-black dark:text-neutral-200">{{ $permission->name }}</span>
                                    </div>
                                </td>

                                <td class="h-px w-auto whitespace-nowrap">
                                    <div class="px-6 py-2">
                                        <span class="text-sm text-black dark:text-neutral-200">{{ $permission->module }}</span>
                                    </div>
                                </td>

                                <td class="h-px w-auto whitespace-nowrap">
                                    <div class="px-6 py-2">
                                        <span class="text-sm text-black dark:text-neutral-200">{{ $permission->created_at->format('M d, Y H:i A') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="h-px w-auto whitespace-nowrap text-end">
                                    <div class="px-6 py-2">
                                        ...
                                        @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('permission edit'))
                                        <a type="button" href="{{ route('permissions.edit',['permission' => $permission->id]) }}"
                                        class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                            Edit
                                        </a>
                                        @endif

                                        @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('permission delete'))
