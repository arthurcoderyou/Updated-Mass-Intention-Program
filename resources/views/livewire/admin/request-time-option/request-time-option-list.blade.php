          ...
          <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-neutral-900 dark:border-neutral-700">
            ...
            <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
              <div>
                ...
                <p class="text-sm text-black dark:text-neutral-400">
                  Listing of request time options
                </p>
              </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                @if(!empty($request_time_options) && count($request_time_options) > 0)
                    @foreach ($request_time_options as $option)
                        ...
                        <tr>
                                    ...
                                    <span class="text-sm text-black dark:text-neutral-200">
                                        @if(!@empty($option->days))
                                            @foreach ($option->days as $day )
                                                <span class="  hover:bg-black hover:text-white inline-flex items-center gap-x-1.5 py-1.5 px-3 rounded-full text-xs font-medium border border-gray-800 text-black dark:border-neutral-200 dark:text-white">
                                                    {{ $day->day->name }}
                                                </span>
                            ...
                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">
                                         {{ \Carbon\Carbon::createFromFormat('H:i:s', $option->time)->format('h:i A') }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">
                                        @if(!empty($option->status) && $option->status == 1)
                                            <div>
                            ...
                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">{{ $option->created_at->format('M d, Y H:i A') }}</span>
                                </div>
                            </td>
                            ...
                            <td class="h-px w-auto whitespace-nowrap text-end">
                                <div class="px-6 py-2">
                                    ...
                                    @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('time edit'))
                                    <a type="button" href="{{ route('request_time_options.edit',['request_time_option' => $option->id]) }}"
                                    class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                        Edit
                                    </a>
                                    @endif

                                    @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('time delete'))
                                    <a href="javascript:void(0)"
                                    onclick="confirm('Are you sure, you want to delete this record?') || event.stopImmediatePropagation()"
                                    wire:click.prevent="deleteOption({{ $option->id }})"
