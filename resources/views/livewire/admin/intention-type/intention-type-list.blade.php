              ...
              <div>
                <div class="inline-flex gap-x-2">
                  ...
                  hover:bg-gray-50 focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-800 dark:border-neutral-700 dark:text-white dark:hover:bg-neutral-700 dark:focus:bg-neutral-700" href="#">
                    ...
                    href="{{ route('intention_types.create') }}">
                    ...
                    @foreach ($intention_types as $type)
                        ...
                        <tr>
                            ...
                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">{{ $type->name }}</span>
                                </div>
                            </td>

                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">{{ $type->created_at }}</span>
                                </div>
                            </td>
                            ...
                            <td class="h-px w-auto whitespace-nowrap">
                                <div class="px-6 py-2">
                                    <span class="text-sm text-black dark:text-neutral-200">
                                        @if(!empty($type->status) && $type->status == 1)
                                            <div>
                            ...
                            <td class="h-px w-auto whitespace-nowrap text-end">
                                <div class="px-6 py-2">
                                    @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('type edit'))
                                    <a type="button" href="{{ route('intention_types.edit',['intention_type' => $type->id]) }}"
                                    class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 focus:outline-none focus:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">
                                        Edit
                                    </a>
                                    @endif

                                    @if(Auth::user()->hasRole('Super Admin') || Auth::user()->can('type delete'))
                                    <a href="javascript:void(0)"
                                    onclick="confirm('Are you sure, you want to delete this record?') || event.stopImmediatePropagation()"
                                    wire:click.prevent="deleteIntentionType({{ $type->id }})"
