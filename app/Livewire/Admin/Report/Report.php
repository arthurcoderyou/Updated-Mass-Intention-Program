...
class Report extends Component
{
    ...
    public function generatePdfV2(int $request_time_option_id = 0,int $day_id = 0){

        $intention_types =  IntentionType::where('status',1)->pluck('id','name')->toArray();
        $days_of_the_week = Day::where('id','>',0)->pluck('id','name')->toArray();
        ...
        $pdf = Pdf::loadView('report.report_without_download', $data);
    ...
    public function updatePriority(int $massIntentionId,int $priority_value,int $intention_id, $direction = null)
    {
        // dd($priority_value);
        ...
        //records the previuos priority
        $prev_priority =  $massIntention->priority;
        ...
        if($priority_value != 0){
            ...
            if(!empty($direction)){
                ...
                if($direction == "move_up"){
                    ...
                    //Updated
                    $prev_mass_intention = MassIntention::where('priority',$priority_value)->where('intention_type',$intention_id)->first();
                    $prev_mass_intention->priority = $prev_priority;
                    $prev_mass_intention->save();
